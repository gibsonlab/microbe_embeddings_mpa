...
SGB_INDEX_DIR_METAPHLAN="/data/bwh-comppath-seq/youn/metaphlan_dset/metaphlan_database"
SGB_INDEX_DIR_PHYLOPHLAN="/data/bwh-comppath-seq/youn/metaphlan_dset/phylophlan_database/processed/dna_only"
if [[ $embed_model_name == evo2* ]]; then
    ...
    singularity exec --nv \
        ...
        --bind "${SGB_INDEX_DIR_METAPHLAN}:/sgb_index_metaphlan" \
        --bind "${SGB_INDEX_DIR_PHYLOPHLAN}:/sgb_index_phylophlan" \
        --env "HF_HOME=/hf_home" \
        ...
        python compute_stacked_gene_embeddings.py \
          ...
          --sgb-marker-index "/sgb_index_metaphlan" "/sgb_index_phylophlan" \
  ...
  python compute_stacked_gene_embeddings.py \
    ...
    --sgb-marker-index "${SGB_INDEX_DIR_METAPHLAN}" "${SGB_INDEX_DIR_PHYLOPHLAN}" \
    --cuda-device-ids "0,1,2,3,4,5,6,7" \
    --output-path "${out_dir}/${out_file}"
fi
