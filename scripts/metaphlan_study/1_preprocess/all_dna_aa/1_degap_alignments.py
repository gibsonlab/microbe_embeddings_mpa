...
def remove_gaps_from_file(alignment_file_bz2: Path, out_file: Path, gene_name: str, seen_ids: Set[str]):
    print(f"De-gapping src: {alignment_file_bz2}")

    with bz2.open(alignment_file_bz2, "rt") as aln_f, open(out_file, "at") as out_f:
...
def main(
    ...
    all_markers_out_path = output_dir / "all_markers.fna"
    ...
    for alignment_file_bz2 in alignments_dir.glob("*.aln.bz2"):
        ...
        remove_gaps_from_file(alignment_file_bz2, all_markers_out_path, gene_name, seen_ids)


if __name__ == "__main__":
    main(
        Path("/data/bwh-comppath-seq/youn/metaphlan_dset/phylophlan_database/PhyloPhlAn_output/msas"),
        Path("/data/bwh-comppath-seq/youn/metaphlan_dset/phylophlan_database/processed/all")
    )
